// Comprehensive content script for Millennium Portal
...
(function () {
    ...
    console.log('[Millennium Sync] DEBUG: SCRIPT VERSION SC-2 (STYLED POPUP)');
    ...
    function updateSyncProgress(current, total, pageName) {
        ...
        // Send progress to popup and background
        if (typeof chrome !== 'undefined' && chrome.runtime) {
            chrome.runtime.sendMessage({
                type: 'SYNC_PROGRESS',
                progress: {
    ...
    async function sendDataToApp(data) {
        try {
            // Send to background script to make the API call (better CORS handling)
