// Google Classroom content script for Millennium Portal Sync
...
(function () {
    ...
    console.log('[Millennium Sync] Script Version: FIX_PAGINATION_V4'); // Updated version log
    ...
    async function scrapeItemsFromPage(courseId, courseName) {
        ...
        // Scroll to load any remaining lazy-loaded content
        ...
        // Find all item links (assignments, materials, posts)
        // These can appear in different URL patterns:
        // - /c/{courseId}/a/{itemId} - stream view
        // - /w/{courseId}/a/{itemId} - classwork view
        // - /c/{courseId}/sa/{itemId} - short answer
        // - /c/{courseId}/m/{itemId} - material
        // - /c/{courseId}/p/{itemId} - post/announcement
        const selectors = [
            'a[href*="/a/"]',   // Assignments
            'a[href*="/sa/"]',  // Short answer assignments  
            'a[href*="/m/"]',   // Materials
            'a[href*="/p/"]',   // Posts/Announcements
        ];

        const itemLinks = document.querySelectorAll(selectors.join(', '));
        console.log(`[Millennium Sync] Found ${itemLinks.length} potential item links`);

        // Debug: Log all links found
        if (itemLinks.length === 0) {
            console.log('[Millennium Sync] No item links found. Page HTML sample:', document.body.innerHTML.substring(0, 2000));
            // Try alternative: look for any links that might be items
            const allLinks = document.querySelectorAll('a[href*="classroom.google.com"]');
            console.log(`[Millennium Sync] Total Classroom links found: ${allLinks.length}`);
            for (const link of Array.from(allLinks).slice(0, 10)) {
                console.log('[Millennium Sync] Link:', link.href);
            }
        }
        ...
        for (const link of itemLinks) {
            ...
            // Extract IDs - item type and ID
            const itemMatch = href.match(/\/(a|sa|m|p)\/([A-Za-z0-9_-]+)/);
            if (!itemMatch) continue;

            // Extract course ID - can be in /c/ or /w/ path
            const linkCourseMatch = href.match(/\/(?:c|w)\/([A-Za-z0-9_-]+)/);

            const [, typeCode, itemId] = itemMatch;
            const linkCourseId = linkCourseMatch?.[1] || courseId;

            // Skip if different course
            ...
            let container = findItemContainer(link);
    ...
    function findLoadMoreButton() {
        ...
        const isSafetyBlocked = (text, btn) => {
            ...
            // SPECIFIC BLOCKS for "View" buttons to avoid navigating away
            if (lower.includes('view instructions')) return true;
            if (lower.includes('view assignment')) return true;
            if (lower.includes('view material')) return true;
            if (lower.includes('view question')) return true;
            if (lower.includes('view class')) return true;
            if (lower.includes('view all')) return true; // usually navigation

            return false;
        };

        // Strategy 1: Button with "Load more" or "View more" text (case-insensitive)
        const buttons = document.querySelectorAll('button');
        for (const btn of buttons) {
            const text = btn.textContent?.trim() || '';
            const lower = text.toLowerCase();

            // Accept "Load more" OR "View more"
            if ((lower === 'load more' || lower.includes('load more') || lower === 'view more') && !isSafetyBlocked(text, btn)) {
        ...
        const labeled = document.querySelectorAll('[aria-label*="more" i]');
        for (const el of labeled) {
            const text = el.getAttribute('aria-label') || '';
            const lower = text.toLowerCase();

            if (!lower.includes('load') && !lower.includes('view')) continue;
        ...
        // Strategy 3: Material design button with load/view more text
        const mdButtons = document.querySelectorAll('.VfPpkd-LgbsSe, [role="button"]');
        for (const btn of mdButtons) {
            const text = btn.textContent?.trim() || '';
            const lower = text.toLowerCase();

            if ((lower.includes('load more') || lower === 'view more') && !isSafetyBlocked(text, btn)) {
                const rect = btn.getBoundingClientRect();
                if (rect.width > 0 && rect.height > 0) {
                    return btn;
                }
    ...
    function extractItemInfo(link, container, defaultType) {
        ...
        // 1. Try to extract from Container (Best Source)
        if (container) {
            const containerText = container.textContent || '';
            const lowerText = containerText.toLowerCase();

            // TYPE DETECTION
            // Look for explicit type labels from the snippet: <div class="PazDv">Assignment</div>
            const typeLabel = container.querySelector('.PazDv')?.textContent?.trim();
            if (typeLabel) {
                if (typeLabel.includes('Assignment')) type = 'assignment';
                else if (typeLabel.includes('Question')) type = 'assignment';
                else if (typeLabel.includes('Material')) type = 'material';
                else if (typeLabel.includes('Announcement')) type = 'announcement';
            }

            // TITLE EXTRACTION
            // Strategy A: Collapsed button aria-label (User snippet: <div ... aria-label="Trig fn 2006 1d">)
            const mainButton = container.querySelector('[role="button"][aria-expanded]');
            if (mainButton && mainButton.getAttribute('aria-label')) {
                title = mainButton.getAttribute('aria-label');
            }

            // Strategy B: Specific title classes from snippet
            if (!title) {
                const titleEl = container.querySelector('.Vu2fZd.Cx437e, .aptkwb, .YVvGBb');
                if (titleEl) title = titleEl.textContent?.trim();
            }

            // Strategy C: Heading tags
            if (!title) {
            ...
            // DUE DATE EXTRACTION
            // Strategy A: Specific date classes (User snippet: .WZkEbf or .qoXqmb)
            const dateEl = container.querySelector('.WZkEbf, .wCDkmf');
            if (dateEl) {
                const text = dateEl.textContent?.trim();
                // Check if it looks like a date/due string
                if (text && (text.includes('Due') || text.match(/\d/))) {
                    dueText = text.replace(/^Due\s*/i, '').trim();
                    dueDate = parseDueDate(dueText);
                }
            }
        }

        // 2. Fallback: Extract from Link (if container failed)
        if (!title) {
            const ariaLabel = link.getAttribute('aria-label') || '';
            // Filter out "View instructions" which is common garbage title
            if (ariaLabel && !ariaLabel.toLowerCase().includes('view instructions')) {
                title = ariaLabel
        ...
        // 4. Detailed Parsing from Container Text (if container exists)
        if (container) {
            const containerText = container.textContent || '';

            // If we missed due date above, try regex on full text
            if (!dueDate) {
                const duePatterns = [
                    /Due\s+([\w\s,:\d]+?)(?:\s*\n|\s*$|\s*\||\s*·)/i,
            ...
            const lowerText = containerText.toLowerCase();
            // User snippet has "Missing" in a span
            ...
            // Description extraction (secondary text)
            // Strategy: Look for text that isn't the title or date or status
            if (!description && title) {
                const descElements = container.querySelectorAll('[class*="description"], [class*="body"], p, .Sn55E');
                for (const el of descElements) {
                    const text = el.textContent?.trim();
                    if (text && text !== title && !text.includes('Due') && text.length > 10) {
                        description = text;
        ...
        return { title, description, descriptionHtml, dueDate, dueText, maxPoints, submissionState, postedTime, type };
        ...
        async function navigateAndScrape(course) {
            // Create a hidden iframe to scrape the course page
            ...
            return new Promise(async (resolve, reject) => {
                try {
                    ...
                    // For other courses, we need to use fetch + DOM parsing or navigate
                    // Since we can't easily scrape dynamic content via fetch, 
                    // we'll scrape the Stream page which has recent items visible

                    // Actually, let's try a different approach:
                    // Open the course page in the current tab, scrape, then return

                    // Store current URL
                    const returnUrl = window.location.href;

                    // Navigate to course's classwork page
                    const classworkUrl = `${baseUrl}/c/${course.id}/w`;

                    // Create a promise that resolves when we've scraped the page
                    const scrapePromise = new Promise((resolveInner) => {
                        // Set up a one-time handler for when we're on the right page
                        const checkAndScrape = async () => {
                            if (window.location.href.includes(`/c/${course.id}`)) {
                                await wait(1500); // Wait for content to load
                                const items = await scrapeItemsFromPage(course.id, course.name);
                                resolveInner(items);
                            }
                        };
                        ...
                        // Check periodically (the page will reload and this script will re-run)
                        // But since this script re-runs on each page, we need a different approach
                    });

                    // Alternative: Just scrape what we can from home page for this course
                    // This is a limitation - we may not get all items without full navigation

                    // For now, let's scrape items visible on the current page that belong to this course
        ...
        async function continueSyncFromState() {
            const result = await chrome.storage.local.get('classroomSyncState');
                ...
                if (courseMatch) {
                    ...
                    if (expectedCourse && courseId === expectedCourse.id) {
                        ...
                        if (nextIndex < courses.length && !syncAborted) {
                            ...
                            // Clear state
                            await chrome.storage.local.remove('classroomSyncState');
        ...
        async function sendToApp(data) {
            return new Promise((resolve, reject) => {
                chrome.runtime.sendMessage({ type: 'CLASSROOM_DATA', data }, response => {
        ...
        `;
            ...
            document.getElementById('mcsAbort')?.addEventListener('click', () => {
                syncAborted = true;
                overlay.remove();
                chrome.storage.local.remove('classroomSyncState');
            });
        }
