(function (global) {
    ...
    /**
     * How far ahead the second timetable page is fetched, to tell a permanent teacher change from a
     * substitute. Mirrors `TEACHER_LOOKAHEAD_DAYS` in `lib/portal-teacher-changes.ts`; this file is
     * a browser-global script and cannot import it.
     */
    ...
    function buildPortalPages(uid, now, options, accountPageUrl) {
        ...
        const timetableFocusDate = parsePortalDate(source.portalDate, now ? new Date(now) : new Date());
        ...
        if (source.includeTimetable !== false) {
            ...
            // The same grid, a fortnight out. Two weeks is one full Week A/B rotation, so this is the
            // same day of the cycle — which is what makes it able to answer whether a new name
            // against a class is a permanent change or someone covering a lesson. Marked optional so
            // the page failing reports nothing rather than marking the whole sync degraded: it
            // answers a question, and a sync that cannot answer it is still a complete sync.
            if (source.includeTeacherLookahead !== false && !source.ultraRun) {
                const lookaheadDate = new Date(timetableFocusDate);
                lookaheadDate.setDate(lookaheadDate.getDate() + TEACHER_LOOKAHEAD_DAYS);
                pages.push({
                    name: `Timetable (+${TEACHER_LOOKAHEAD_DAYS}d)`,
                    url: appendPortalDate(`${baseUrl}/timetable.asp?uid=${uid}`, lookaheadDate),
                    type: 'timetableLookahead',
                    lookaheadDate: toIsoDateString(lookaheadDate),
                    optional: true
                });
            }
        }
        ...
        const focusDate = timetableFocusDate;
    ...
    function applyPageResult(allData, result) {
        ...
        if (page.type === 'timetableLookahead') {
            const lookahead = scrapeTimetable(doc);
            // An empty grid is not evidence about anything, and storing one would let a failed
            // lookahead masquerade as "the slot is gone", which reads as a confirmed change.
            if (lookahead.weekA.length || lookahead.weekB.length) {
                allData.timetableLookahead = { date: page.lookaheadDate || null, ...lookahead };
            }
        }
    ...
    async function scrapePortalSnapshot(options) {
        ...
        let completed = 0;
        // Counted separately from `failures`, which only carries the pages whose loss degrades the
        // sync. An optional page that failed still did not succeed, and the page tally must say so.
        let succeeded = 0;
        const failures = [];
        ...
        await limitedMap(pages, concurrency, async (page, index) => {
            try {
                ...
                const message = String(error?.message || error || 'Portal page failed')
                    ...
                    .slice(0, 240);
                // An optional page answers a question rather than carrying data the dashboard needs,
                // so losing one is not a degraded sync and must not be reported as one.
                if (!page.optional) {
                    failures.push({
                        page: page.name,
                        section: page.type,
